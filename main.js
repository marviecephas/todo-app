...
            console.log(filtering.style.color)
...
            value.querySelector('.check-box-bg').style.background = (active.includes(todoCon.querySelector('.todo').innerHTML)) ? 'hsl(235, 24%, 19%)' : value.querySelector('.check-box').querySelector('.check-box-bg').style.background;
            value.querySelector('.check-box').querySelector('.check-box-border').style.background = (active.includes(todoCon.querySelector('.todo').innerHTML)) ? 'hsl(237, 14%, 26%)' : value.querySelector('.check-box').querySelector('.check-box-border').style.background;
        })
...
        });
        todoCon.style.borderRadius = '5px';
...
        });
        todoCon.querySelectorAll('.todo-main').forEach(value => {
            value.querySelector('.check-box-bg').style.background = (active.includes(todoCon.querySelector('.todo').innerHTML)) ? 'hsl(0, 0%, 100%)' : value.querySelector('.check-box').querySelector('.check-box-bg').style.background;
            value.querySelector('.check-box').querySelector('.check-box-border').style.background = (active.includes(todoCon.querySelector('.todo').innerHTML)) ? 'hsl(236, 33%, 92%)' : value.querySelector('.check-box').querySelector('.check-box-border').style.background;
        })
        todoCon.style.borderRadius = '5px';
...
event.dataTransfer.setData('text', event.target.closest('.todo-main'))
...
    const draggedIndex = Array.prototype.indexOf.call(todoCon.children, draggedElement);
    const targetIndex = Array.prototype.indexOf.call(todoCon.children, targetElement);
...
            itemsNo.innerHTML = (itemsNo.innerHTML > 0) ?  Number(itemsNo.innerHTML) - 1 : 0;
        })
      checkBg.appendChild(checked); checkBox.appendChild(checkBorder);
       checkBox.appendChild(checkBg); subTodoMain.appendChild(checkBox);
